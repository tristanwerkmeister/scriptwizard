...
| `{PRESET_SLUG}` | Derived from `{PRESET_NAME}`: lowercase, spaces to hyphens, strip special characters (e.g., "Aviation Beat" → `aviation-beat`). If stripping produces a slug that already exists (e.g., "City Herald/Health" and "City Herald-Health" both become `city-herald-health`), append `-2` or ask the user to choose a different name. |
...
```
Your script presets:
- **Aviation Beat** — Skyline Journal, English (UK), updated 3 Apr
- **Local Politics** — Riverside Gazette, English (US), updated 12 Mar
- **Global Wire Health** — Global Wire, English (International), updated 1 Feb
